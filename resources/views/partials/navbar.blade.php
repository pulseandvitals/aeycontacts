        ...
        <li class="nav-item dropdown no-arrow">
            <a class="nav-link dropdown-toggle" href="#" id="userDropdown" role="button"
                ...
                <span class="mr-2 d-none d-lg-inline text-white-600 small">
                    <small class="{{ Auth::user()->role == 'jobseeker' ? 'text-success' : 'text-info' }}">
                        {{ Auth::user()->role == 'jobseeker' ?  'Job Seeker'
                            : (Auth::user()->role == 'employer' ? 'Employer' : 'Guest') }}
                    </small>
                        {{ Auth::user()->name }}
                </span>
                <img class="img-profile rounded-circle"
                    src="{{ Auth::user()->current_avatar
                        ?  asset('/images/'.Auth::user()->current_avatar)
                        : asset('images/default.jpg') }}"
                >
            </a>
            <!-- Dropdown - User Information -->
            @php $isEmpty = App\Models\Profile::where('profile_id',Auth::user()->id)->count(); @endphp
            <div class="dropdown-menu dropdown-menu-right shadow animated--grow-in"
                aria-labelledby="userDropdown">
                <a class="dropdown-item" href="{{ $isEmpty != 0 ? route('profile.show',Auth::user()->id)
                    : route('profile.add') }}">
